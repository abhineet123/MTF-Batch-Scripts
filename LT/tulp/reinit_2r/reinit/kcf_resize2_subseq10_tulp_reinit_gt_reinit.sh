...
KCF_SETTINGS="kcf_padding 1 kcf_lambda 1e-4 kcf_output_sigma_factor 0.1 kcf_interp_factor 0.02 kcf_kernel_sigma  0.5 kcf_number_scales 33 kcf_scale_step 1.02 kcf_scale_model_max_area 512 kcf_scale_sigma_factor 0.25 kcf_scale_learning_rate 0.01 kcf_enableScaling 0 kcf_resize_factor 2"
...
TRACKING_DATA_FNAME=kcf_50r30i4u_2r_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 1 $KCF_SETTINGS		
		let SEQ_ID=SEQ_ID+1 		
	done
	let ACTOR_IDS_IDX=ACTOR_IDS_IDX+1
done
