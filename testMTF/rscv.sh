...
N_SEQS=${#SEQ_IDS[@]}
SEQ_IDX_ID=0
DIAG_OUT_PREFIX=rscv256b_2
while [ $SEQ_IDX_ID -lt ${N_SEQS} ]; do
	SEQ_ID=${SEQ_IDS[$SEQ_IDX_ID]}
	ACTOR_ID=${ACTOR_IDS[$SEQ_IDX_ID]}
	$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID diag_out_prefix $DIAG_OUT_PREFIX $SETTINGS $DIAG_SETTINGS $SCV_SETTINGS diag_3d 0
	$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID diag_out_prefix $DIAG_OUT_PREFIX $SETTINGS $DIAG_SETTINGS $SCV_SETTINGS diag_3d 1
