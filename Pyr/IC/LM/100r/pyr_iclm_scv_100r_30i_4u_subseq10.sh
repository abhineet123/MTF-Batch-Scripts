...
TRACKING_DATA_FNAME=pyrSRiclmcw1C1_scv100r30i4u_8_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		while [ $SUBSEQ_ID -lt $(($N_SUB_SEQ)) ]; do		
			...
			$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS mtf_am scv reinit_on_failure 0 $PYR_SETTINGS $IC_SETTINGS $SCV_SETTINGS $HOM_SETTINGS init_frame_id $INIT_ID			
			let SUBSEQ_ID=SUBSEQ_ID+1
		done
		$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS mtf_am scv reinit_on_failure 1 $PYR_SETTINGS $IC_SETTINGS $SCV_SETTINGS $HOM_SETTINGS		
		let SEQ_ID=SEQ_ID+1 		
	done
	let ACTOR_IDS_IDX=ACTOR_IDS_IDX+1
done
