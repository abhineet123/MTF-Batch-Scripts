...
TRACKING_DATA_FNAME=pffclm500d5s5s6s7s8s11car_ngf50r30i300a1b4u_sl3m_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
MAX_RETRIES=10
N_RETRIES=0
...
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		while [ $SUBSEQ_ID -lt $(($N_SUB_SEQ)) ]; do		
			...
			timeout 30m $EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 0 $PF_SETTINGS $FC_SETTINGS $NGF_SETTINGS $SL3_SETTINGS init_frame_id $INIT_ID			
			EXIT_STATUS=$?
if [ $EXIT_STATUS -eq 139 ] || [ $EXIT_STATUS -eq 124 ]; then
		...
		timeout 30m $EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 1 $PF_SETTINGS $FC_SETTINGS $NGF_SETTINGS $SL3_SETTINGS			
		EXIT_STATUS=$?
if [ $EXIT_STATUS -eq 139 ] || [ $EXIT_STATUS -eq 124 ]; then
