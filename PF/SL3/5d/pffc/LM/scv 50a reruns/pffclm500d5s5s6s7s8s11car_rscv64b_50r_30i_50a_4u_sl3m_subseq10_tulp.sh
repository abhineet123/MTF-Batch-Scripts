...
RSCV_SETTINGS="scv_use_bspl 0 scv_n_bins 64 scv_preseed 0 scv_pou 1 scv_weighted_mapping 0 scv_mapped_gradient 0 scv_affine_mapping 1 scv_once_per_frame 0 scv_approx_dist_feat 0 likelihood_alpha 50 likelihood_beta 1 dist_from_likelihood 0"
...
TRACKING_DATA_FNAME=pffclm500d5s5s6s7s8s11car_rscv64b50r30i50a4u_sl3m_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
MAX_RETRIES=10
N_RETRIES=0
...
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		while [ $SUBSEQ_ID -lt $(($N_SUB_SEQ)) ]; do		
			...
			timeout 30m $EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 0 $PF_SETTINGS $FC_SETTINGS $RSCV_SETTINGS $SL3_SETTINGS init_frame_id $INIT_ID			
			EXIT_STATUS=$?
if [ $EXIT_STATUS -eq 139 ] || [ $EXIT_STATUS -eq 124 ]; then
		...
		timeout 30m $EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 1 $PF_SETTINGS $FC_SETTINGS $RSCV_SETTINGS $SL3_SETTINGS			
		EXIT_STATUS=$?
if [ $EXIT_STATUS -eq 139 ] || [ $EXIT_STATUS -eq 124 ]; then
