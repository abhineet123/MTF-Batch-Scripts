...
CCRE_SETTINGS="ccre_n_bins 24 ccre_pre_seed 10 ccre_pou 1 ccre_symmetrical_grad 0 ccre_n_blocks 1 likelihood_alpha 100 likelihood_beta 1 dist_from_likelihood 0"
...
TRACKING_DATA_FNAME=pffclm500d5s5s6s7s8s11car_ccre24b50r30i100a1b4u_sl3m_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
MAX_RETRIES=10
N_RETRIES=0
...
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	N_FRAMES_FNAME="$DB_ROOT_PATH/$ACTOR/n_frames.txt"	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		while [ $SUBSEQ_ID -lt $(($N_SUB_SEQ)) ]; do		
			...
			timeout 30m $EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 0 $PF_SETTINGS $FC_SETTINGS $CCRE_SETTINGS $SL3_SETTINGS init_frame_id $INIT_ID
			EXIT_STATUS=$?			
		...
		timeout 30m $EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 1 $PF_SETTINGS $FC_SETTINGS $CCRE_SETTINGS $SL3_SETTINGS			
		EXIT_STATUS=$?
		if [ $EXIT_STATUS -eq 139 ] || [ $EXIT_STATUS -eq 124 ]; then
