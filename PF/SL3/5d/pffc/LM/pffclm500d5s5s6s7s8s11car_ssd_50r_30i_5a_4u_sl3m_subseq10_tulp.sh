...
SSD_SETTINGS="ssd_show_template 0 ssd_forgetting_factor 0.50 likelihood_alpha 5 likelihood_beta 1 dist_from_likelihood 0"
...
TRACKING_DATA_FNAME=pffclm500d5s5s6s7s8s11car_ssd50r30i5a4u_sl3m_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
MAX_RETRIES=10
N_RETRIES=0
...
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		while [ $SUBSEQ_ID -lt $(($N_SUB_SEQ)) ]; do		
			...
			timeout 30m $EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 0 $PF_SETTINGS $FC_SETTINGS $SSD_SETTINGS $SL3_SETTINGS init_frame_id $INIT_ID			
			EXIT_STATUS=$?
if [ $EXIT_STATUS -eq 139 ] || [ $EXIT_STATUS -eq 124 ]; then
		...
		timeout 30m $EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 1 $PF_SETTINGS $FC_SETTINGS $SSD_SETTINGS $SL3_SETTINGS			
		EXIT_STATUS=$?
if [ $EXIT_STATUS -eq 139 ] || [ $EXIT_STATUS -eq 124 ]; then
