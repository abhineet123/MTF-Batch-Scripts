...
TRACKING_DATA_FNAME=pf1kd5s5s6s7s8s11car_ncc50r30i50a4u_sl3m_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
MAX_RETRIES=10
N_RETRIES=0
...
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		while [ $SUBSEQ_ID -lt 1 ]; do		
			...
			$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 0 $PF_SETTINGS $NCC_SETTINGS $SL3_SETTINGS init_frame_id $INIT_ID			
			if [ $? -eq 139 ]; then
		...
		$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 1 $PF_SETTINGS $NCC_SETTINGS $SL3_SETTINGS			
		if [ $? -eq 139 ]; then
