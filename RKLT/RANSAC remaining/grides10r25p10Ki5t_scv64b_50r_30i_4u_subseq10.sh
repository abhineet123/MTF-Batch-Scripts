...
EST_SETTINGS="est_method 0 est_ransac_reproj_thresh 5 est_n_model_pts 4 est_max_iters 10000 est_max_subset_attempts 300 est_use_boost_rng 0 est_confidence 0.995 est_refine 1 est_lm_max_iters 10"
SCV_SETTINGS="scv_hist_type 0 scv_n_bins 64 scv_preseed 0 scv_pou 1 scv_weighted_mapping 0 scv_mapped_gradient 0 scv_affine_mapping 1 scv_once_per_frame 0 scv_approx_dist_feat 0"
...
TRACKING_DATA_FNAME=grides10r25p10Ki5t_scv64b50r30i4u_8_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		while [ $SUBSEQ_ID -lt $(($N_SUB_SEQ)) ]; do		
			...
			$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS mtf_am scv reinit_on_failure 0 $GRID_SETTINGS $EST_SETTINGS $ESM_SETTINGS $SCV_SETTINGS $HOM_SETTINGS  init_frame_id $INIT_ID			
			let SUBSEQ_ID=SUBSEQ_ID+1
		done
		$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS mtf_am scv reinit_on_failure 1 $GRID_SETTINGS $EST_SETTINGS $ESM_SETTINGS $SCV_SETTINGS $HOM_SETTINGS 			
		let SEQ_ID=SEQ_ID+1 		
	done
	let ACTOR_IDS_IDX=ACTOR_IDS_IDX+1
done
