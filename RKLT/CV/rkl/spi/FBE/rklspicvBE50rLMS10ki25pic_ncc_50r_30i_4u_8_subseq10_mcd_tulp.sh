...
RKL_SETTINGS="rkl_sm ic rkl_enable_spi 1 rkl_enable_feedback 1 rkl_failure_detection 0 rkl_failure_thresh 15.0"
...
TRACKING_DATA_FNAME=rklspicvBE50rLMS10ki25pic_ncc50r30i4u_8_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		while [ $SUBSEQ_ID -lt $(($N_SUB_SEQ)) ]; do		
			...
			$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 0 $GRID_SETTINGS $EST_SETTINGS $RKL_SETTINGS $HOM_SETTINGS $IC_SETTINGS init_frame_id $INIT_ID			
			let SUBSEQ_ID=SUBSEQ_ID+1
		done
		$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 1 $GRID_SETTINGS $EST_SETTINGS $RKL_SETTINGS $HOM_SETTINGS $IC_SETTINGS $MCD_REINIT_SETTINGS	
