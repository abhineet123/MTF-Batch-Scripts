...
RKL_SETTINGS="rkl_sm esm rkl_enable_spi 1 rkl_enable_feedback 1 rkl_failure_detection 0 rkl_failure_thresh 15.0"
HOM_SETTINGS="hom_corner_based_sampling 1 hom_normalized_init 1"
ESM_SETTINGS="sec_ord_hess 0 esm_jac_type 1 esm_hess_type 2 leven_marq 0 lm_delta_init 0.01 lm_delta_update 10 esm_chained_warp 1 enable_learning 0"
...
TRACKING_DATA_FNAME=rklspicvBE50rLMS10ki25pes_ncc50r30i4u_8_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		while [ $SUBSEQ_ID -lt $(($N_SUB_SEQ)) ]; do		
			...
			$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 0 $GRID_SETTINGS $EST_SETTINGS $RKL_SETTINGS $HOM_SETTINGS $ESM_SETTINGS init_frame_id $INIT_ID			
			let SUBSEQ_ID=SUBSEQ_ID+1
		done
		$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 1 $GRID_SETTINGS $EST_SETTINGS $RKL_SETTINGS $HOM_SETTINGS $ESM_SETTINGS $MCD_REINIT_SETTINGS	
