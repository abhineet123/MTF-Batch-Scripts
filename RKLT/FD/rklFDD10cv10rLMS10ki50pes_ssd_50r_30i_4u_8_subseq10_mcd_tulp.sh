...
RKL_SETTINGS="rkl_sm esm rkl_enable_spi 0 rkl_enable_feedback 1 rkl_failure_detection 1 rkl_failure_thresh 10.0"
...
TRACKING_DATA_FNAME=rklFD10cv10rLMS10ki50pes_ssd50r30i4u_8_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		while [ $SUBSEQ_ID -lt $(($N_SUB_SEQ)) ]; do		
			...
			$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 0 $GRID_SETTINGS $EST_SETTINGS $RKL_SETTINGS $HOM_SETTINGS $FC_SETTINGS init_frame_id $INIT_ID			
			let SUBSEQ_ID=SUBSEQ_ID+1
		done
		$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS reinit_on_failure 1 $GRID_SETTINGS $EST_SETTINGS $RKL_SETTINGS $HOM_SETTINGS $FC_SETTINGS $MCD_REINIT_SETTINGS	
