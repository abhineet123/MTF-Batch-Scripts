...
PYR_SETTINGS="pyr_sm ic pyr_no_of_levels 3 pyr_scale_factor 0.5 pyr_scale_res 1 pyr_show_levels 0"
IC_SETTINGS="sec_ord_hess 0 ic_update_ssm 0 leven_marq 0 lm_delta_init 0.01 lm_delta_update 10 ic_chained_warp 1 enable_learning 0 ic_hess_type  0 leven_marq 1 lm_delta_init 0.01 lm_delta_update 10"
RKL_SETTINGS="rkl_sm ic rkl_enable_spi 0 rkl_enable_feedback 1 rkl_failure_detection 0 rkl_failure_thresh 15.0"
GRID_SETTINGS="grid_sm pyr grid_am rscv grid_ssm 2 grid_ilm 0 grid_res 10 grid_patch_size 25 grid_patch_res 0 grid_patch_centroid_inside 1 grid_fb_err_thresh 0 grid_fb_reinit 1 grid_dyn_patch_size 0 grid_reset_at_each_frame 1 grid_show_trackers 0 grid_show_tracker_edges 0 grid_use_tbb 0 grid_pyramid_levels 2 grid_use_min_eig_vals 0 grid_min_eig_thresh 1e-4"
...
SCV_SETTINGS="scv_use_bspl 0 scv_n_bins 256 scv_preseed 0 scv_pou 1 scv_weighted_mapping 0 scv_mapped_gradient 0 rscv_affine_mapping 1 rscv_once_per_frame 0 rscv_approx_dist_feat 0"
...
TRACKING_DATA_FNAME=rklpyriclm10rLMS25p10Ki5t_rscv50r30i4u_8_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		while [ $SUBSEQ_ID -lt $(($N_SUB_SEQ)) ]; do		
			...
			$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS mtf_am rscv reinit_on_failure 0 $GRID_SETTINGS $RKL_SETTINGS $EST_SETTINGS $IC_SETTINGS $PYR_SETTINGS $SCV_SETTINGS $HOM_SETTINGS  init_frame_id $INIT_ID			
			let SUBSEQ_ID=SUBSEQ_ID+1
		done
		$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS mtf_am rscv reinit_on_failure 1 $GRID_SETTINGS $RKL_SETTINGS $EST_SETTINGS $IC_SETTINGS $PYR_SETTINGS $SCV_SETTINGS $HOM_SETTINGS 			
		let SEQ_ID=SEQ_ID+1 		
	done
	let ACTOR_IDS_IDX=ACTOR_IDS_IDX+1
done
