...
NN_SETTINGS="nn_n_samples 10000 nn_index_type 1 nn_load_index 0 nn_save_index 0 nn_search_type 0 nn_max_iters 1 nn_ssm_sigma_ids 2 nn_ssm_mean_ids 0 nn_pix_sigma 0 nn_n_checks 50 nn_additive_update 0 nn_show_samples 0 nn_add_samples_gap 0 nn_remove_points 0"
GNN_SETTINGS="nn_gnn_degree 250 nn_gnn_max_steps 100 nn_gnn_cmpt_dist_thresh 10000 nn_gnn_random_start 0 nn_gnn_verbose 0 nn_fgnn_index_type 1"
...
TRACKING_DATA_FNAME=fgnnkdt10k2s_ssd50r1i4u_8_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		while [ $SUBSEQ_ID -lt $(($N_SUB_SEQ)) ]; do		
			...
			$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS mtf_am ssd reinit_on_failure 0 $NN_SETTINGS $GNN_SETTINGS $HOM_SETTINGS init_frame_id $INIT_ID			
			let SUBSEQ_ID=SUBSEQ_ID+1
		done
		$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS mtf_am ssd reinit_on_failure 1 $NN_SETTINGS $GNN_SETTINGS $HOM_SETTINGS
		let SEQ_ID=SEQ_ID+1 		
	done
	let ACTOR_IDS_IDX=ACTOR_IDS_IDX+1
done
