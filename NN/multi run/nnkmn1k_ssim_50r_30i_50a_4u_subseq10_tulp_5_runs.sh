...
TRACKING_DATA_FNAME=nnkmn1k2s_ssim50r1i4u_8_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		IFS=$saveIFS
		RUN_ID=1
		while [ $RUN_ID -le $(($N_RUNS)) ]; do
		SUBSEQ_ID=0
			while [ $SUBSEQ_ID -lt $(($N_SUB_SEQ)) ]; do		
				INIT_ID=${SUBSEQ_START_IDS[$SUBSEQ_ID]}
				$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME$RUN_CONN$RUN_ID $SETTINGS mtf_am ssim reinit_on_failure 0 $NN_SETTINGS $HOM_SETTINGS init_frame_id $INIT_ID			
				let SUBSEQ_ID=SUBSEQ_ID+1
			done
			$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME$RUN_CONN$RUN_ID $SETTINGS mtf_am ssim reinit_on_failure 1 $NN_SETTINGS $HOM_SETTINGS	
			let RUN_ID=RUN_ID+1
		done
		let SEQ_ID=SEQ_ID+1 		
	done
