...
TRACKING_DATA_FNAME=nnkmn1k2s_ssd50r1i4u_8_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		SUBSEQ_ID=0
			while [ $SUBSEQ_ID -lt $(($N_SUB_SEQ)) ]; do		
				...
				$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME$RUN_CONN$RUN_ID $SETTINGS mtf_am ssd reinit_on_failure 0 $NN_SETTINGS $HOM_SETTINGS init_frame_id $INIT_ID			
				let SUBSEQ_ID=SUBSEQ_ID+1
			done
			$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME$RUN_CONN$RUN_ID $SETTINGS mtf_am ssd reinit_on_failure 1 $NN_SETTINGS $HOM_SETTINGS
			let RUN_ID=RUN_ID+1
		done
		let SEQ_ID=SEQ_ID+1 		
	done
