...
IC_SETTINGS="sec_ord_hess 0 ic_update_ssm 0 leven_marq 1 lm_delta_init 0.01 lm_delta_update 10 ic_chained_warp 1 enable_learning 1 ic_hess_type  0"
...
TRACKING_DATA_FNAME=iclmcw1C1_mcpca50r30i4u_8_0
N_ACTORS=${#ACTOR_IDS[@]}
ACTOR_IDS_IDX=0
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do		
		...
		while [ $SUBSEQ_ID -lt $(($N_SUB_SEQ)) ]; do		
			...
			$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS mtf_am mcpca reinit_on_failure 0 $IC_SETTINGS $HOM_SETTINGS $PCA_SETTINGS init_frame_id $INIT_ID			
			let SUBSEQ_ID=SUBSEQ_ID+1
		done
		$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS mtf_am mcpca reinit_on_failure 1 $IC_SETTINGS $HOM_SETTINGS $PCA_SETTINGS
		let SEQ_ID=SEQ_ID+1 		
	done
	let ACTOR_IDS_IDX=ACTOR_IDS_IDX+1
done
