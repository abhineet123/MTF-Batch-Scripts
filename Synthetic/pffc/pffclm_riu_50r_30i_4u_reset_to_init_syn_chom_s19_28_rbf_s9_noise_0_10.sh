...
AM_SETTINGS="likelihood_alpha 1000 likelihood_beta 0 dist_from_likelihood 0"
...
TRACKING_DATA_FNAME=pffclm500d5s0s1s2s3s4car_riu50r30i1e3a4u_8_0
N_ACTORS=${#ACTOR_IDS[@]}
...
while [ $ACTOR_IDS_IDX -lt ${N_ACTORS} ]; do	
	...
	while [ $SEQ_ID -lt ${N_SEQ} ]; do
		...
		while [ $SYN_SSM_SIGMA_IDS_IDX -lt ${N_SYN_SSM_SIGMA_IDS} ]; do	
			...
			$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS mtf_am riu reinit_on_failure 0 $PF_SETTINGS $FC_SETTINGS $HOM_SETTINGS $AM_SETTINGS $SYN_SETTINGS syn_ssm_sigma_ids $SYN_SSM_SIGMA_ID syn_add_noise 0
			
			$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS mtf_am riu reinit_on_failure 0 $PF_SETTINGS $FC_SETTINGS $HOM_SETTINGS $AM_SETTINGS $SYN_SETTINGS syn_ssm_sigma_ids $SYN_SSM_SIGMA_ID syn_add_noise 1		

			$EXEC_NAME actor_id $ACTOR_ID source_id $SEQ_ID tracking_data_fname $TRACKING_DATA_FNAME $SETTINGS mtf_am riu reinit_on_failure 0 $PF_SETTINGS $FC_SETTINGS $HOM_SETTINGS $AM_SETTINGS $SYN_SETTINGS syn_ssm_sigma_ids $SYN_SSM_SIGMA_ID syn_add_noise 1 syn_ilm rbf syn_am_sigma_ids 9
